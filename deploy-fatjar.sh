#!/usr/bin/bash
usage(){
   echo 'usage: CONTAINER=<container> so-deploy-fatjar <target>'
   echo '       CONTAINER=<container> so-deploy-fatjar [OPTIONS]'
   echo 'target'
   echo '  where the fatjar artifact locate, or file path'
   echo '[OPTIONS]:'
...
#########################################################################
### start deploy
### check origin path, back it up for rollback.
#########################################################################
deploytarget(){
   local target=$1
   ...
   ls -l $filesystem_workingdir/$deploy_target 2> /dev/null  ## dotnot show err 

   if [[ $target_op = web ]];then
      if [ -f $filesystem_workingdir/$deploy_target ];then
      ...
      ## back it up for later rollback (if app.jar/*-standalone.jar, rollback)
      echo ./rollback.sh -k 6 $filesystem_workingdir
      ./rollback.sh -k 6 $filesystem_workingdir
